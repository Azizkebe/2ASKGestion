...
class CreatePermission extends Component
{
    ...
    if(isset($this->id_employe)){
        ...
        $this->nombre_de_jour = $this->donnees_employe->nombre_de_jour_permis ?? 0;
        $this->nombre_de_jour_conge_annuel = $this->donnees_employe->nombre_de_jour_annuel ?? 0;
    }
    if(isset($this->date_depart)and(isset($this->date_retour)))
    {
        ...
        try {
            ...
            if($reussi){
                ...
                if($permission->id_statut_permission === '2')
                {
                    ...
                    $this->donnees_employe->update(['nombre_de_jour_permis'=> $this->nbr_jour_restant,
                    'nombre_de_jour_annuel'=> $this->nbr_jour_restant_annuel]);

                }
                else
                {

                    $this->donnees_employe->update(['nombre_de_jour_permis'=> $this->nombre_de_jour]);
            ...
            return redirect()->route('permission.liste')->with('Bravo, la permission est autorisée avec succes');
